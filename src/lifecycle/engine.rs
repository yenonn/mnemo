...
impl LifecycleEngine {
    pub fn check_and_fire(conn: &Connection, manager: &mut TierManager) -> Vec<HookResult> {
        ...
        // Guard: disabled
        let enabled = config::get_bool(conn, "lifecycle_enabled", true).unwrap_or(true);
        if !enabled {
            return results;
        }

        let idle_threshold: i64 =
            config::get_i64(conn, "lifecycle_idle_threshold", 60).unwrap_or(60);
        let decay_rate: f64 = config::get_f64(conn, "lifecycle_decay_rate", 0.1).unwrap_or(0.1);
        let consolidate_on_flush =
            config::get_bool(conn, "lifecycle_consolidate_on_flush", true).unwrap_or(true);

        let now = Utc::now().timestamp_millis();
        let last_activity = config::get_i64(conn, "lifecycle_last_activity", 0).unwrap_or(0);
        let idle_seconds = (now - last_activity) / 1000;

        // 1. Buffer overflow check
        let mut stmt =
            match conn.prepare("SELECT COUNT(*) FROM memories WHERE memory_type = 'working'") {
                Ok(s) => s,
                Err(_) => {
        ...
        if idle_seconds >= idle_threshold {
            ...
            if consolidated_count > 0 || !consolidate_on_flush {
                results.push(HookResult::SessionEnd {
                    consolidated_count,
                    new_episodic_id,
                });
            }
            ...
            match recall::auto_recall(conn) {
                Ok(recalled) => {
                    if recalled > 0 {
                        results.push(HookResult::SessionStart {
...
mod tests {
    ...
    fn test_check_and_fire_idle_consolidates() {
        ...
        store
            .insert("working", "Test memory", 0.5, "test", &[])
            .unwrap();

        // Set last_activity to 2 minutes ago
        let two_mins_ago = Utc::now().timestamp_millis() - (2 * 60 * 1000);
        crate::lifecycle::config::set(&conn, "lifecycle_last_activity", &two_mins_ago.to_string())
            .unwrap();
